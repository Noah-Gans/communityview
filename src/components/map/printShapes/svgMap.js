...
const catalogImageIcon = (file, alt = '') => (
  <span
    style={{
      display: 'inline-flex',
      width: '100%',
...
export const printCatalogToolIcons = {
  ...
  shape_pin: catalogImageIcon('location-pin-parking.svg', 'Pin'),

  // shape_* tools
  shape_square: catalogImageIcon('garage-car.svg', 'Square'),
  shape_circle: catalogImageIcon('camera.svg', 'Circle'),
  shape_home: catalogImageIcon('house-chimney.svg', 'Home'),
  shape_airport: catalogImageIcon('plane-alt.svg', 'Airport'),
  shape_mountain: catalogImageIcon('skiing.svg', 'Mountain'),
  shape_mountains2: catalogImageIcon('skiing-nordic.svg', 'Mountains'),
  shape_trail: catalogImageIcon('hiking.svg', 'Trail'),
  shape_park: catalogImageIcon('table-picnic.svg', 'Park'),
  shape_hiker: catalogImageIcon('hiking.svg', 'Hiker'),
  shape_school: catalogImageIcon('school.svg', 'School'),
  shape_bank: catalogImageIcon('farm.svg', 'Bank'),
  shape_climb1: catalogImageIcon('horse-saddle.svg', 'Climb'),
  shape_gym: catalogImageIcon('skiing-nordic.svg', 'Gym'),
  shape_climb2: catalogImageIcon('horse-saddle.svg', 'Climb 2'),
  shape_iceCream: catalogImageIcon('camera.svg', 'Ice cream'),
  shape_lake: catalogImageIcon('swimmer.svg', 'Lake'),
  shape_shop: catalogImageIcon('camera.svg', 'Shop'),
  shape_groceryStore: catalogImageIcon('farm.svg', 'Grocery'),
...
const pointIconRenderer = (file) => ({
  ...
  stroke = '#111827',
  strokeWidth = 3,
  fillOpacity = 1, // badge/background opacity
  iconOpacity = 1,
  iconScale = 0.64,
  logoColor = '#111827',
  strokeOpacity = 1,
  ...
  const ringWidth = Math.max(1, Number(strokeWidth || 1));
  const scale = Math.max(0.2, Math.min(1, Number(iconScale ?? 0.64)));
  const iconSize = 100 * scale;
  const iconInset = (100 - iconSize) / 2;
  const hex = String(logoColor || '#111827').replace('#', '');
  const safeHex = hex.length === 3 ? hex.split('').map((c) => c + c).join('') : hex.padStart(6, '0').slice(0, 6);
  ...
  const tintId = `logo-tint-${file.replace(/[^a-z0-9]/gi, '-')}`;
  return (
      ...
      <defs>
        <filter id={tintId}>
          <feColorMatrix
            ...
            result="alphaMask"
          />
          <feFlood floodColor={`rgb(${Math.round(r * 255)}, ${Math.round(g * 255)}, ${Math.round(b * 255)})`} result="tint" />
          <feComposite in="tint" in2="alphaMask" operator="in" result="tintedIcon" />
        </filter>
      </defs>
      {withBadge && (
        <>
          <circle cx="50" cy="50" r="46" fill={fill} fillOpacity={Math.max(0, Math.min(1, fillOpacity ?? 1))} />
          <circle
            cx="50"
            cy="50"
            r={Math.max(6, 46 - ringWidth / 2)}
            fill="none"
            stroke={stroke}
            strokeWidth={ringWidth}
            strokeOpacity={Math.max(0, Math.min(1, strokeOpacity ?? 1))}
          />
        </>
      )}
      <image
        href={`/logos_for_print/${file}`}
        x={withBadge ? String(iconInset) : '0'}
        y={withBadge ? String(iconInset) : '0'}
        width={withBadge ? String(iconSize) : '100'}
        height={withBadge ? String(iconSize) : '100'}
        opacity={Math.max(0, Math.min(1, iconOpacity ?? 1))}
        filter={`url(#${tintId})`}
...
// Replace legacy point SVGs with the new downloaded icon set for on-map rendering.
Object.assign(svgMap, {
  pin: pointIconRenderer('location-pin-parking.svg'),
  square: pointIconRenderer('garage-car.svg'),
  circle: pointIconRenderer('camera.svg'),
  home: pointIconRenderer('house-chimney.svg'),
  airport: pointIconRenderer('plane-alt.svg'),
  mountain: pointIconRenderer('skiing.svg'),
  mountains2: pointIconRenderer('skiing-nordic.svg'),
  trail: pointIconRenderer('hiking.svg'),
  park: pointIconRenderer('table-picnic.svg'),
  hiker: pointIconRenderer('hiking.svg'),
  school: pointIconRenderer('school.svg'),
  bank: pointIconRenderer('farm.svg'),
  climb1: pointIconRenderer('horse-saddle.svg'),
  gym: pointIconRenderer('skiing-nordic.svg'),
  climb2: pointIconRenderer('horse-saddle.svg'),
  iceCream: pointIconRenderer('camera.svg'),
  lake: pointIconRenderer('swimmer.svg'),
  shop: pointIconRenderer('camera.svg'),
  groceryStore: pointIconRenderer('farm.svg'),
