 ...
 * @param {string} query
 * @param {{ limit?: number, countyCodes?: string[], offsetId?: string|number|null, maxLimit?: number }} options
 */
export async function searchParcels(query, options = {}) {
  ...
  try {
    ...
    else if (searchIntent === 'address') params.append('fields[address][ilike]', trimmedQuery);
    ...
    console.error('Regrid parcel search failed:', error);
    return { results: [], hasMore: false, nextOffsetId: null };
  }
}
